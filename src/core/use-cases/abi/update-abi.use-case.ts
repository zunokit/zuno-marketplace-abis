...
export class UpdateAbiUseCase {
  ...
  async execute(input: UpdateAbiUseCaseInput): Promise<UpdateAbiUseCaseOutput> {
    // Track ABI update start
    SentryTracker.addBreadcrumb("abi", "ABI update started", "info", {
      abiId: input.abiId,
    });
    ...
    try {
      ...
      SentryTracker.trackAbiUpdated(input.abiId, input.changeLog || "ABI updated");
      ...
      SentryTracker.trackError("abi_update", "Failed to update ABI", error as Error);
      throw error;
    }
  }
