...
export class RateLimitService {
   ...
   */
  private static isOriginAllowed(origin: string | undefined, allowedOrigins: string[]): boolean {
  ...
  static async checkLimit(
    ...
    try {
      ...
      if (config.restrictions?.allowedOrigins) {
        if (!this.isOriginAllowed(request.origin, config.restrictions.allowedOrigins)) {
          const result: RateLimitResult = {
            allowed: false,
            tier,
            limit: 0,
