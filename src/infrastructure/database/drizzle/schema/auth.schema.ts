...
export const apiKey = pgTable("api_key", {
  ...
  // ⚠️ SECURITY: API keys are HASHED using SHA-256 before storage
  // Never store plaintext keys - Better Auth handles hashing automatically
  start: text("start"), // First 8 chars for UI display (e.g., "zuno_abc") - NOT sensitive
  prefix: text("prefix"), // Custom prefix (e.g., 'zuno_', 'sk_live_', 'pk_test_')
  key: text("key").notNull().unique(), // ⚠️ HASHED API key (SHA-256 hash) - NEVER plaintext
